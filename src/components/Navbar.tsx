...
type NavItem = {
    ...
    id?: string;
    isSpecial?: boolean;
    children?: { key: string; href: string; id: string; isSpecial?: boolean }[];
};

const NAVIGATION: NavItem[] = [
    { key: 'home', href: '/', id: 'home' },
    {
        key: 'toolsAndCalculators',
        children: [
            { key: 'heightPredictor', href: '/height-predictor', id: 'height-predictor' },
            { key: 'heightWeightPercentileCalculator', href: '/height-weight-percentile-calculator', id: 'height-weight-percentile-calculator' },
            { key: 'idealBodyWeightCalculator', href: '/ideal-body-weight-calculator', id: 'ideal-body-weight-calculator' },
            { key: 'averageHeightByCountry', href: '/average-height-by-country', id: 'average-height-by-country' },
            { key: 'heightDifferenceCalculator', href: '/height-difference-calculator', id: 'height-difference-calculator' },
            { key: 'imageToHeight', href: '/image-to-height', id: 'image-to-height' },
        ]
    },
    { key: 'about', href: '/about', id: 'about' },
    { key: 'blogs', href: '/blogs', id: 'blogs' },
];
...
const Navbar: React.FC = () => {
    ...
    // Normalize active page calculation regardless of locale prefix
    const cleanPath = pathname.replace(/^\/(de|en)(\/|$)/, '/');
    const activePage = cleanPath === '/' ? 'home' : cleanPath.split('/').filter(Boolean).pop() || 'home';
    ...
    return (
        <>
            ...
            <header className={`h-[60px] shrink-0 border-b flex items-center justify-between px-4 sm:px-8 xl:px-12 z-300 top-0 transition-all duration-300 ${scrolled
                ...
                {/* Logo */}
                <Link href={getLocalizedHref('/')} className="flex items-center gap-3 cursor-pointer shrink-0">
    ...
    "
                        title={`${t('switchTo')} ${unitSystem === 'metric' ? t('imperialFtIn') : t('metricCm')}`}
                    ...
                    >
                        ...
                        <span className="inline-block px-2 py-1 rounded-md bg-accent/10 border border-accent/20 group-hover:bg-accent transition-colors">
                            <span className="text-[11px] sm:text-xs font-extrabold uppercase tracking-widest text-accent group-hover:text-bg whitespace-nowrap">
                                {unitSystem === 'metric' ? t('metricCm') : t('imperialFtIn')}
                            </span>
                        </span>
                    </button>
                        ...
                        >
                            <div className="flex flex-col gap-2">
                                ...
                                {NAVIGATION.map((item, idx) => {
                                                ...
                                                <div className={`overflow-hidden transition-all duration-[350ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${mobileToolsOpen ? 'grid grid-rows-[1fr] opacity-100' : 'grid grid-rows-[0fr] opacity-0'}`}>
                                                    <div className="overflow-hidden">
                                                        <div className="flex flex-col gap-1 pl-4 border-l border-white/10 ml-4 mb-2 mt-1">
                                                            {item.children.map(child => (
                                                                <Link key={child.id} href={getLocalizedHref(child.href)} onClick={() => setIsNavMenuOpen(false)}>
                                    ...
                                    return (
                                        <Link key={item.id} href={getLocalizedHref(item.href!)} onClick={() => setIsNavMenuOpen(false)}>
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="px-2.5 py-0.5 rounded-full bg-white/10 border border-white/10 text-[10px] uppercase tracking-wider text-white group-hover:bg-white/20 transition-colors">
                                            {unitSystem === 'metric' ? t('imperialFtIn') : t('metricCm')}
                                        </span>
                                    </div>
